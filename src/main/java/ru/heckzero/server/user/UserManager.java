...
public class UserManager {                                                                                                                  //yes, this class name ends in 'er' fuck off, Egor ;)
    ...
    public enum UserType {IN_GAME, ONLINE_GAME, ONLINE_CHAT, ONLINE_GAME_OR_CHAT, IN_BATTLE, NPC, HUMAN, POLICE}
    ...
    private static boolean areInSameLoc(User user1, User user2) {                                                                           //if users are at a same location
    ...
    private static boolean areInSameRoom(User user1, User user2) {                                                                          //if users are in a same room
    ...
    public static void loginUser(Channel ch, String login, String userCryptedPass) {                                                        //check if the user can log in and set him online
        ...
        if (!StringUtils.equals(serverCryptedPass, userCryptedPass)) {                                                                      //passwords don't match
            ...
            ch.writeAndFlush(errMsg).addListener(ChannelFutureListener.CLOSE);
            HistoryUser.addIms(HistoryCodes.LOG_WRONG_PASSWORD, user, (String)ch.attr(AttributeKey.valueOf("sockStr")).get());              //add history record about failure login
            return;
        }
